	...
	public class Factory : Opc.Factory
	{
		...
		public static object Connect(Opc.URL url, Opc.ConnectData connectData)
		{
			...
			if (connectData == null || connectData.LicenseKey == null)
			{
				try
				{
					return OpcCom.Interop.CreateInstance(guid, url.HostName, credentials, connectData?.UseConnectSecurity ?? false);
				}
				catch (Exception e)
				{
			...
			else
			{
				try
				{
					return OpcCom.Interop.CreateInstanceWithLicenseKey(guid, url.HostName, credentials, connectData.LicenseKey, connectData.UseConnectSecurity);		
				}
				catch (Exception e)
				{
					throw new Opc.ConnectFailedException(e);
